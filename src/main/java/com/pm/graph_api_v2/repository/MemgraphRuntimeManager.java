...
public class MemgraphRuntimeManager {

    private static final Logger log = LoggerFactory.getLogger(MemgraphRuntimeManager.class);
    ...
    public void initialize() {
        ...
        if (!memgraphProperties.isSyncGraphStateOnStartup()) {
            log.info("memgraph graph-state synchronization is skipped at startup");
            return;
        }

        try (Session session = newSession()) {
            session.run("CREATE INDEX ON :" + Neo4jQuerySupport.NODE_LABEL + "(node_id)").consume();
            if (memgraphProperties.isClearProjectionOnStartup()) {
    ...
    private void clearProjection(Session session) {
        ...
        while (true) {
            ...
            if (total % 50_000 == 0) {
                log.info("memgraph projection nodes cleared: {}", total);
            }
        }
        log.info("memgraph projection nodes cleared: {}", clearedNodes.get());
    }
    ...
    public boolean isAvailable() {
        try {
            ...
            log.debug("Memgraph connectivity check failed: {}", ex.getMessage());
            return false;
        }
    }
    ...
    private void syncNodes(Session session) {
        ...
        graphRepository.forEachNodeBatch(SYNC_BATCH_SIZE, nodes -> {
            ...
            if (total % 50_000 == 0) {
                log.info("memgraph projection nodes synced: {}", total);
            }
        });
        log.info("memgraph projection nodes synced: {}", syncedNodes.get());
    }
    ...
    private void syncRelationships(Session session) {
        ...
        graphRepository.forEachEdgeBatch(SYNC_BATCH_SIZE, edges -> {
            ...
            if (total % 50_000 == 0) {
                log.info("memgraph projection source edges scanned: {}, relationships synced: {}", total, projectedRelationships.get());
            }
        });

        flushRelationshipBatch(session, batch);
        log.info("memgraph projection source edges scanned: {}, relationships synced: {}", sourceEdges.get(), projectedRelationships.get());
    }
